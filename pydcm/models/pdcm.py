...
def pdcm_fmri_priors(A, B, C, D, options):
    """
    Construct prior expectations (pE) and prior covariances (pC) 
    for the P-DCM model based on A, B, C matrices and modeling options.

    Parameters
    A : np.ndarray
        Intrinsic connectivity matrix
    B : np.ndarray
        Bilinear modulation matrix
    C : np.ndarray
        Input matrix (n x m)
    D : np.ndarray
        Nonlinear modulation matrix (n x n x l)
    options : dict
        Dictionary of optional hyperparameters. Keys can include:
        - 'precision': inverse variance scaling for A
        - 'decay': decay constant for prior expectation of A
    
    Returns
    pE : dict
        Dictionary of prior expectations.
    pC : np.ndarray
        Diagonalized prior covariance matrix.
    x : np.ndarray
        Initial state vector of shape (n, 6).
    """

    pE = {}
    pC = {}

    #### number of regions, given by number of rows in A matrix ####
    n = A.shape[0]

    ##### connectivity priors and intitial states ####
    #### Havlicek 2015, supplementary info 5 ####

    #### initial states (6), 6 states are fixed for DCM model ####
    x  = zeros((n, 6))
...
def fx_fmri_pdcm(x, u, P, M=None):
    """
    Computes the derivatives of the neuronal and hemodynamic state variables
    in the P-DCM model for fMRI.

    Parameters:
    x : ndarray
        State vector of shape (n_regions, 6), where each column represents:
        0 - Excitatory neuronal activity
        1 - Vasoactive signal
        2 - log(rCBF)
        3 - log(blood volume)
        4 - log(deoxyhemoglobin content)
        5 - Inhibitory neuronal activity

    u : ndarray
        External input vector of shape (n_inputs, )

    P : dict
        Parameter dictionary with keys:
        - 'A', 'B', 'C': connectivity matrices
        - 'sigmas', 'mus', 'lambdas', etc.: region-wise parameters
        - hemodynamic parameters like 'transit', 'gain', etc.

    M : dict or None
        Optional dictionary for model options (not used directly here).

    Returns:
    f : ndarray
        Derivatives of state vector flattened in Fortran order.
    """
