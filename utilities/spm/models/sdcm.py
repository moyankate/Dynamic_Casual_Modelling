...
# === NEW: pad or trim u to match number of bilinear terms ===
    if np.ndim(P['B']) > 2:
        n_bilinear = P['B'].shape[2]
        if len(u) < n_bilinear:
            u = np.concatenate([u, np.zeros(n_bilinear - len(u))])
        elif len(u) > n_bilinear:
            u = u[:n_bilinear]
    ...
    if x.shape[-1] == 5:
        ...
        n_inputs = P['C'].shape[1]
        f[:, 0] = EE @ x[:, 0] + P['C'] @ u[:n_inputs].reshape(-1) # edited so that number of inputs does not affect calculation
...
#### New function written by Kate ####

def spm_fx_fmri_nargout(x, u, P, M=None, want_jacobian=False):
    """
    Wrapper to optionally return Jacobian like MATLAB's nargout=2
    """
    # Call your original function (assumes it returns only f)
    f = spm_fx_fmri(x, u, P, M)

    # If the caller wants the Jacobian, calculate or fake it
    if want_jacobian:
        dfdx = None  # TODO: implement or set to zero matrix as a placeholder
        return f, dfdx
    else:
        return f
