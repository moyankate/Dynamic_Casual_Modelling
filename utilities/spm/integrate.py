...
#        M['g'] = g
    ...
    for i in range(size(t)):
        ...
        # input dependent changes in Jacobians
        # ----------------------------------------------------------------------
        q = spm.unvec(x, M['x'])
        # fx, dfdx   = M['f'](q,U['u'][t[i],:],P,M, nargout = 2) nargout = 2 is for MATLAB, does not work in Python, use below instead
        fx_dfdx = M['f'](q, U['u'][t[i],:], P, M)
        if isinstance(fx_dfdx, tuple):
            fx, dfdx = fx_dfdx
        else:
            fx = fx_dfdx
            dfdx = np.zeros((fx.size, fx.size))  # placeholder or real Jacobian later
