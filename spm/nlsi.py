from pydcm import spm
import pydcm
from pydcm.globs import *
from pydcm.utils import *
from pydcm import wrappers
...
def spm_nlsi_GN(M, U=[], Y={}):
    ...
    for k in range(M['Nmax']):
        ...
        for m in range(m_step_iters):
            ...
            for i in range(nh):
                # print(f"len(Q): {len(Q)}")
                # print(f"len(h): {len(h.ravel())}")
                # print(f"current i: {i}")          to check if the index are in range, after print, len of Q and h should be the same to avoid errors
